...
def main() -> None:
    p = argparse.ArgumentParser(description="Plot Im(rho_21) vs detuning (MHz).")
    p.add_argument(
        "datafile",
        nargs="?",
    ...
    p.add_argument(
        "--save",
        default="im_vs_detuning.png",
        help="Output image filename (default: im_vs_detuning.png)",
    )
    p.add_argument(
        "--noshow",
        action="store_true",
    ...
    detuning_MHz, _, im_rho21 = load_rho21_file(path)

    plt.figure()
    plt.plot(detuning_MHz, im_rho21)
    plt.xlabel("Detuning (MHz)")
    plt.ylabel("Im(rho_21)")
    plt.title("Im(rho_21) vs detuning")
    plt.grid(True)
